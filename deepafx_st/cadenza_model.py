...
class CadenzaModel(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.eps_scheduler = EpsilonScheduler(epsilon=0.0005)
        ...
        #     system = CadenzaSystem.load_from_checkpoint(self.hparams.encoder_ckpt)
        #     self.encoder = system.encoder
        #     self.encoder_embed_dim = system.encoder.embed_dim
        # else:
        ...
        # self.recon_losses = torch.nn.ModuleDict()
        # for recon_loss in self.recon_losses:
    ...
    def forward(
        ...
        if data_sample_rate != self.dsp_sample_rate:
            ...
            if y is not None:
                y_enc = torchaudio.transforms.Resample(data_sample_rate, self.dsp_sample_rate).to(x.device)(y)
        else:
            x_enc = x
            y_enc = y
